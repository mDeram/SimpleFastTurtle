...
static int next_token(struct ListNode **cur_node, struct TokenNode **cur_token);
static struct Statement *parse_statement(struct ListNode **p_cur_node,
                                         struct TokenNode **p_cur_token);
static struct Expression *parse_expression(struct ListNode **p_cur_node,
                                           struct TokenNode **p_cur_token);
static struct Expression *parse_nested_expression(struct TokenNode **expression_arr,
                                                  int start,
                                                  int stop);
...
static void parse(struct List *s_tree_token, struct List *s_list_token)
{
    ...
    {
        list_push(s_tree_token, parse_statement(p_cur_node, p_cur_token));
    }
}

static int next_token(struct ListNode **p_cur_node, struct TokenNode **p_cur_token)
{
    printf("ID: %d\n", (*p_cur_token)->id);
                                         ...
                                         struct TokenNode **p_cur_token)
{
    ...
    unsigned long int tmp_line = (*p_cur_token)->line;
    while ((*p_cur_token)->id == TOK_SEP_SEMI)
    {
        if (next_token(p_cur_node, p_cur_token)) /* ; */
    ...
    if ((*p_cur_token)->type == TOK_TYPE_KEY)
    {
        switch((*p_cur_token)->id)
        {
            ...
            case TOK_KEY_ELIF:
                ...
                if (next_token(p_cur_node, p_cur_token)) /* for/while/if/elif */
                ...
                while ((*p_cur_token)->id != TOK_SEP_CBS)
                {
                    list_push(s_statement->expressions,
                                parse_expression(p_cur_node, p_cur_token)); /*todo check if expression not null*/
                }
                ...
                while ((*p_cur_token)->id != TOK_SEP_CBE)
                {
                    struct Statement *s_new_statement
                            = parse_statement(p_cur_node, p_cur_token);
                    if (s_new_statement)
            ...
            case TOK_KEY_FN:
                ...
                if (next_token(p_cur_node, p_cur_token)) /* fn */
                    error_printd(ERROR_PARSER_INVALID_STATEMENT_BLOCK_END, &s_statement->token->line);

                s_statement->name = *p_cur_token;

                /*todo error not identifier */
                if (next_token(p_cur_node, p_cur_token)) /* identifier (name) */
                    error_printd(ERROR_PARSER_INVALID_STATEMENT_BLOCK_START, &s_statement->token->line);

                while ((*p_cur_token)->id != TOK_SEP_CBS)
                {
                    list_push(s_statement->expressions,
                                parse_expression(p_cur_node, p_cur_token));
                }
            ...
            case TOK_KEY_ELSE:
                ...
                if (next_token(p_cur_node, p_cur_token)) /* else */
                    error_printd(ERROR_PARSER_INVALID_STATEMENT_BLOCK_START, &s_statement->token->line);
            ...
            case TOK_KEY_VAR:
                if (next_token(p_cur_node, p_cur_token)) /* var */
                    error_printd(ERROR_PARSER_INVALID_STATEMENT_BLOCK_END, &s_statement->token->line);
            ...
            case TOK_KEY_BREAK:
                if (next_token(p_cur_node, p_cur_token)) /* break */
                    error_printd(ERROR_PARSER_INVALID_STATEMENT_BLOCK_END, &s_statement->token->line);
                break;
                                           ...
                                           struct TokenNode **p_cur_token)
{
        ...
        && (*p_cur_token)->id != TOK_SEP_COMMA)
    {
        ...
        if (bracket_stack < 0 || next_token(p_cur_node, p_cur_token))
            break;
    }
